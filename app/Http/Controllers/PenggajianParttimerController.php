...
use App\Imports\PenggajianParttimerImport;
use PDF;
use PhpOffice\PhpSpreadsheet\Writer\Pdf as WriterPdf;

class PenggajianParttimerController extends Controller
{
    public function index()
    {
        $id = Auth::user()->id;
        $penggajian_parttimer = DB::table('penggajian_parttimer')->whereNull('penggajian_parttimer.deleted_at')->get();
        return view('penggajian.penggajian_parttimer.index', compact('penggajian_parttimer'));
    }
    public function import(Request $request){
        ...
        Excel::import(new PenggajianParttimerImport($data), $request->file('file')->store('temp'));
        return back();
    }
    ...
    {
        $id = Crypt::decrypt($id);
        $data = DB::table('penggajian_parttimer')->leftJoin('pegawai','penggajian_parttimer.nip','pegawai.nip')->where('penggajian_id',$id)->whereNull('penggajian_parttimer.deleted_at')->first();
        return view('penggajian.penggajian_parttimer.detail', compact('data','id'));
    }
    public function cetak($id)
    {
        $id = Crypt::decrypt($id);
        $data = DB::table('penggajian_parttimer')->leftJoin('pegawai','penggajian_parttimer.nip','pegawai.nip')->where('penggajian_id',$id)->whereNull('penggajian_parttimer.deleted_at')->first();
        return view('penggajian.penggajian_parttimer.cetak', compact('data'));
    }
